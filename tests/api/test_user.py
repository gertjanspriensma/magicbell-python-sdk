...
class TestCreateUser:
    async def test_error_is_handled_properly(self, magicbell_client: belfry_magicbell.MagicBell):
    ...
    async def test_response_is_parsed_properly(self, magicbell_client: belfry_magicbell.MagicBell):
        response = await magicbell_client.users.create_user(
            belfry_magicbell.WrappedUser(user=belfry_magicbell.User(email="foo@example.com"))
        )

        assert response.user.email == "foo@example.com"

    async def test_server_error_handling(self, magicbell_client: belfry_magicbell.MagicBell):
        with pytest.raises(errors.MagicBellHTTPServerError) as exc_info:
            await magicbell_client.users.create_user(
                belfry_magicbell.WrappedUser(user=belfry_magicbell.User(email="500@example.com"))
            )
...
class TestDeleteUserDetailed:
    async def test_response_is_accessible(self, magicbell_client: belfry_magicbell.MagicBell):
